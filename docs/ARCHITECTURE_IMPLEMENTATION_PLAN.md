...
作成日: 2026-09-14  
状態: 実装進行中。M0のコード照合と基準テスト、M1のSession境界第一段階、M2のStorage batch・Repository移行・保存revision隔離の第一段階に加え、§18のR00〜R04・R06〜R08を実装済み（[§18.11 実施記録](#1811-実施記録2026-09-14)）。本文中で実装済みと明記していない新しいモジュール、API、試験、性能目標は提案である。


**機能追加と操作性の計画は別冊:** [FEATURE_IMPLEMENTATION_PLAN.md](./FEATURE_IMPLEMENTATION_PLAN.md)（A1〜E4）。この文書は構造改善（R00〜R20）を扱う。両者が同じファイルへ触る箇所は別冊§8にある。

**最新の着手計画:** [§18「主要コード読解後の詳細実行計画」](#18-主要コード読解後の詳細実行計画2026-09-14追補)。確認済み不具合3件を先に修正し、R00〜R20の作業単位・依存関係・担当境界・合格条件を定義した。§1〜17の初版記録と実装済み履歴は保持する。実施した範囲と残りは[§18.11](#1811-実施記録2026-09-14)にある。
...
この更新時点で実施済みなのは、M0のコード照合・Node/build検証とブラウザsmoke、M1のSession境界とCommand/Controller抽出、M2のStorage batch・Repository移行・保存revision隔離第一段階、M5のうち素材取り込みの非同期規約と原本の不変性（§18のR00〜R04・R06〜R08）である。M1の画面責務分離、M2の保存中Asset使用権の登録、M3・M4の実装、M5のAssetManagerと容量管理、M6以降、性能改善は未完了である。詳細は§18.11にある。
...
### 18.11 実施記録（2026-09-14）

本節はR00〜R20のうち、実際に実装・検証した範囲の記録である。未着手のRは§18.4の計画のままであり、ここに書いていないことは実装されていない。

| ID | 状態 | 実装したこと | 検証 |
|---|---|---|---|
| R00 | 完了 | 基準の固定。Nodeテスト83件成功、build成功を確認。ブラウザ検証の依存を用意した | `npm test` / `npm run build` / `npm run test:browser` |
| R01 | 完了 | `shot()` / `scene()` / `sceneName()`をmodelへ置き、`project()`・`split()`・UIのScene追加が同じ生成規約を使う | Node（Scene追加→Undo→Redo→保存/読込、splitのShot）＋実ブラウザの`[data-act="scene"]`経路 |
...
- Nodeテスト117件成功。`npm run build`成功。`npm run test:browser`成功（ページエラー0件）。
- ブラウザ検証はこの環境のChromiumで実行した。`npm install --no-save playwright` と `CHROMIUM_EXECUTABLE=/opt/pw-browsers/chromium-1194/chrome-linux/chrome` を使う。Playwrightは配布物の依存ではない。
- §6.3で未実行としていたブラウザsmokeは実行できるようになった。M0のブラウザ検証はこの範囲で解消した。
- Windowsのペン入力、実機の音声遅延、実プリンター、複数タブのIndexedDB排他は依然として未検証である。
- 性能の比較測定（§18.9の1〜5）は未実施。R12以降の部分更新に着手する時点で、固定データと計測区間を揃えてから行う。

**この実装で変えていないこと**

- Project JSON Version 5、保存形式、Snapshotの保持規則、Undo/Redoの履歴規則。
- 画面更新は全体`render()`のまま。`changes`は契約として返すが、まだ部分更新には使っていない。
- Store.editの複製範囲（R14の部分複製は未着手）。
- 出力（紙面・Animatic）の制御構造。`app.js`のexport経路はそのまま。

**次に着手する順序**

1. R05（保存・保持整理・GCの直列化、実行中素材の保護）。R07で旧原本の保持が意味を持つようになったため、GCの到達可能性と保護登録を先に固める。
2. R09（AssetManager）。R08で波形キャッシュの区間キーを入れたので、容量予算と解放をここでまとめる。
3. R10〜R12（View抽出と部分更新）。R04の`changes`をそのまま無効化の入力に使う。

### 18.12 計測でわかったことと、計画の修正（2026-09-16）

R番号の作業とは別に、操作の追加と操作感の改善を行った。着手前に費用を測り、
§18.4の優先順位を一部見直した。測定はこの環境（Linux + Node 22 / Chromium 1194）
の中央値で、環境が変われば値も変わる。

| 処理 | 500 Panel | 2000 Panel |
|---|---|---|
| 編集1回（深い複製 + validate + 変更比較） | 0.74 ms | 0.79 ms |
| Undo | 0.08 ms | 0.19 ms |
| flatten | 0.07 ms | 0.08 ms |
| validate単体 | 0.37 ms | 0.49 ms |
| JSON.stringify | 42 ms | 236 ms |
| load + validate | 116 ms | 385 ms |
| repository.save（MemoryStorage） | 299 ms | 648 ms |
| JSONサイズ | 3.7 MB | 14.8 MB |

**R14（部分複製）の優先度を下げる。** 編集そのものは2000 Panelでも1ms未満で、
保存1回の1/300しかない。「毎回Project全体を複製している」のは事実だが、現状では
誰も待たされていない費用である。R14は履歴とcurrentの不変性を保つ難しさに対して
取れる時間が小さいので、他の作業の後に回す。着手するなら、先に2000 Panelを超える
規模か、複製の対象が増える変更（Panelあたりのデータ増）が実際に出てからにする。

**保存は「速くする」より「止めない」問題である。** 2000 PanelではJSON化だけで
236msメインスレッドを止め、連続編集中は4秒ごとに走る。分割シリアライズと
yieldで応答を保つ方向が、保存形式を変えずに効く。まだ未着手。

**ブラウザ側の既存メトリクスは内訳の判断に使えない。** `browser-smoke.mjs`の
`measure()`は2フレーム待ちを含むため、20〜45msという値はほぼ床に張り付いている。
個別の費用は、同期的に測る別の計測（今回の選択コストの測定のように）で取る。

#### 実施した追加（R番号の外）

| 内容 | 効果・検証 |
|---|---|
| 数値入力の横ドラッグ（`src/ui/number-scrub.js`） | AEのホットテキストと同じ操作。1回のドラッグが1段のUndo。純粋関数をNodeで4件検証 |
| オニオンスキン | 前後のコマを赤と青で薄く重ねる。`drawing.draw`へ描画オプションを追加。Projectには保存しない |
| Panelのコピー / カット / 貼り付け | `commands.pastePanels`。IDを振り直し、参照する素材メタデータも持ち運ぶ。Node 4件 |
| 選択時の部分更新 | TreeとStripを作り直さない。500 Panelで選択9.1→3.5ms、矢印キー6.4→3.1ms |
| パネルの出し入れ（`src/ui/docks.js`） | 画面を4つ＋1つのドックに分け、開くパネルと前面のパネルだけを状態として持つ。DOMとの約束は`data-dock`/`data-tabs`/`data-body`/`data-splitter`の4つ。Node 10件（前面の切り替え、空のドックの畳み方、壊れた保存からの復帰） |
| キー操作の表（`src/ui/shortcuts.js`） | 効くキーと画面に出す説明を1か所から配る。ショートカットのパネルも同じ表から作るので、実装と説明が離れない。Node 8件 |
| パネルの表と作業レイアウト（`src/ui/panels.js`） | どのパネルがどのドックに属し、ウィンドウメニューにどう並ぶかをデータで持つ。「閉じられるパネルは必ずメニューから戻せる」をNodeで検証（6件） |
| ポップアップメニュー（`src/ui/menu.js`） | ファイルとウィンドウの2つで共有。開いているメニューは常に1つ |

部分更新は「直前に描いたProjectのオブジェクトと同一か」だけで判定する。
`Store.edit`は変更があったときにしか新しいProjectを作らないので、この判定は
表示内容の同一性とそのまま一致する。変更種類ごとの無効化（§18.6の表）へ進むのは、
この判定で足りなくなってからでよい。

`src/ui/`へ出したのは、いずれも「DOMの形だけに依存し、Projectを知らない」部分である。
§10の`*-view.js`（Tree・Stage・Inspector・Timelineの表示そのものを`app.js`から
剥がす計画）は未着手で、これらはその前段にあたる。

#### 次に検討する順序（更新）

1. R05（保存・保持整理・GCの直列化、実行中素材の保護）。R07で旧原本の保持が
   意味を持つようになったため、到達可能性の判定を先に固める。
2. Animatic出力の内容固定（可変の`rows`/`images`/`resolved`を読んでいる）と、
   紙面PNG連番の逐次ZIP化。R16/R17の一部を先に取る。
3. 保存を止めないための分割シリアライズ。
4. R09（AssetManager）、R11/R12（変更種類ごとの部分更新）。
