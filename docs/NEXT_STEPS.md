...
更新日：2026-09-13。初期実装 `8b928b0` を基準に作成し、P0（保存・復旧とデータ基盤）の実装後に状態を更新した。これは今後の実装計画であり、未実装機能の完成報告ではない。次の対象はP1（描画とPanel操作）。
...
| Model | Scene/Shot/Panel、整数フレーム、v2検証、v1→v2 Migration、Asset参照 | Project設定UI、素材を実際に生成するUI（P1） |
...
SE/BGM欄は音声ファイルの配置機能ではない。ブラウザ再生は動画ファイル書き出しではない。現在のUIを完成画像と同等と扱わない。自動保存はブラウザのIndexedDBであり、OS上のファイルへは保存ボタンで別途書き出す必要がある。
...
2. ✅ IndexedDBへ最大8世代のスナップショットを保持。変更から1.2秒（最長8秒）で遅延保存し、ヘッダーに「未保存の変更 / 自動保存中 / ブラウザに保存 時刻 / 失敗」を表示する。失敗は失敗として表示し、未保存のまま次の編集で再試行する。
...
| 保存 | JSON化・素材書込み・保存完了を分け、サイズも記録（現状：500 Panelのメモリ保存 中央値72.45ms、読込116.72ms、2.98MB） |
...
2. `npm test`と`npm run build`で開始時点を確認。現在のUIを操作して描画とPanel操作の不便を再現する。
3. P1は画像取り込みから着手する。`project.assets`と`panel.image`、`ProjectRepository.putAsset/getAsset`は用意済みなので、Drawing Adapterでの描画とInspectorの操作を足す。素材の実データをプロジェクトJSONへ戻さない。
4. P1の完了条件を満たしたらP2へ進む。時間が残る場合もフェーズを飛び越えて機能を増やさない。
