...
> **現状注記（2026-09-19）:** この資料はP5直後の引き継ぎを含む履歴資料です。現在のmainでは素材込みの .contb Bundle、ExportSnapshot、編集境界、Asset安全性が追加されています。最新の優先順位・完了条件・判断ゲートは [NEXT_PLAN.md](NEXT_PLAN.md) を参照してください。
...
| 出力 | 紙コンテ（PNG連番ZIP・印刷/PDF・進捗と中止）、Animatic（音つきWebMの実時間録画、フレーム厳密なPNG連番ZIP） | 文字埋め込みPDF、実時間より速いオフライン動画書き出し、H.264等の選択 |
