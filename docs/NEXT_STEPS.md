...
更新日：2026-09-14。初期実装 `8b928b0` を基準に作成し、P0（保存・復旧とデータ基盤）、P1（描画とPanel操作）、P2（Timeline EngineとCamera Track）、P3（音配置・波形・同期再生）、P4（紙コンテを本番出力へ）の実装後に状態を更新した。これは今後の実装計画であり、未実装機能の完成報告ではない。次の対象はP5（Animatic出力とWindowsデスクトップ化）。
...
| 右Inspector/Camera/書き出し | 選択内容に応じた低頻度設定の収納 | タブ化と可変幅を実装。書き出し設定は紙コンテのダイアログに集約し、プロジェクトへ保存 |
...
| 出力 | PNG連番のZIP、印刷経由の画像ベースPDF、1ページずつの進捗と中止 | ネイティブPDF（文字埋め込み）、Animatic動画、音声ミックス（P5） |
...
### P5 — Animatic出力とWindowsデスクトップ化（次回の主対象）
...
| Exporter | exporter.js（ページ単位の進捗・中止・ZIP） | 動画出力、Workerでの生成 |
...
2. `npm test`と`npm run build`で開始時点を確認。再生・紙コンテ出力を一通り操作し、動画として書き出せないことを確認する。
3. P5は映像と音声を同じフレーム評価関数で回すところから始める。`cameraAt`・`draw`・`scheduleFor`を出力側から呼び、`exporter.js`の進捗/中止をそのまま使う。ブラウザ側は`MediaRecorder`か画像連番＋外部エンコーダのどちらを採るかを、ライセンス・保守・容量の観点で先に比較して記録する。
4. Windowsのペン入力・ファイル上書き・音声・メモリを実機で比べてからネイティブシェルを選ぶ。先に特定フレームワークを前提にしない。
