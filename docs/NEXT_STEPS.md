...
更新日：2026-09-14。初期実装 `8b928b0` を基準に作成し、P0（保存・復旧とデータ基盤）、P1（描画とPanel操作）、P2（Timeline EngineとCamera Track）の実装後に状態を更新した。これは今後の実装計画であり、未実装機能の完成報告ではない。次の対象はP3（音配置・波形・同期再生）。
...
| 下部Timeline | Panel・台詞・音・Cameraの時間関係を一望 | fps目盛・選択範囲・Cameraトラックを実装。音声/台詞トラックはP3 |
| 右Inspector/Camera/書き出し | 選択内容に応じた低頻度設定の収納 | タブ化と可変幅を実装。書き出し設定の収納は紙コンテのダイアログのまま |
| 音声波形とCameraキー | 同一時刻で再生・スクラブ・書き出し | Cameraキーは任意時刻で編集可。音声は文字注記のみ（P3） |
...
| Timeline | Timeline Engine（範囲・目盛・スナップ・Zoom・追従）、端ドラッグのリップル尺変更、スクラブ、選択範囲表示、Cameraトラック | 音声/台詞トラック、クリップのドラッグ移動、複数トラックの階層表示 |
...
| 紙コンテ | 項目・コマ数・画像幅・余白・文字サイズ・ヘッダー、全キーを通るCamera軌道/開始終了枠/矢印/HOLD | A4縦固定、テキスト列別幅、長文継続、進捗とキャンセル |
...
| Camera Animation | model.js（キー編集・補間・describeCameraをUI/紙/再生で共有） | イージング、動画出力との共有 |
...
2. `npm test`と`npm run build`で開始時点を確認。現在のUIを操作して、音を置けないこと（SE/BGM欄は文字の注記にすぎない）を確認する。
3. P3はAudio Engineを`src/audio.js`として新設し、`ProjectRepository`の素材ストア（`putAsset`/`getAsset`、kind:"audio"）とTimeline Engineの区間計算を使う。波形はキャッシュし、UIスレッドで作り直さない。
4. P3の完了条件を満たしたらP4へ進む。時間が残る場合もフェーズを飛び越えて機能を増やさない。
