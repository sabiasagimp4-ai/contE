...
更新日：2026-09-13。初期実装 `8b928b0` を基準に作成し、P0（保存・復旧とデータ基盤）とP1（描画とPanel操作）の実装後に状態を更新した。これは今後の実装計画であり、未実装機能の完成報告ではない。次の対象はP2（Timeline EngineとCamera Track）。
...
|---|---|---|
| 左のScene/Shot/Panelツリー | 構成の把握と移動。選択を全ビューで同期 | Scene/Shot名編集と自動スクロールを実装。折りたたみ状態の保持は未実装 |
| 中央の大きな描画面 | 描画を主役にし、タイミング編集と往復しない | ブラシ/消しゴム/筆圧/ズーム/パン/画像取り込みを実装。レイヤー・図形・カラーは未実装 |
| 描画面下のPanel一覧 | 前後の絵を比較しながら連続追加 | 範囲選択とドラッグ順序変更を実装。複数Shotの横断表示は未実装 |
| 下部Timeline | Panel・台詞・音・Cameraの時間関係を一望 | 現在はPanelトラックのみ。時間目盛/複数トラックを追加する（P2） |
| 右Inspector/Camera/書き出し | 選択内容に応じた低頻度設定の収納 | タブ化と可変幅を実装。書き出し設定の収納は紙コンテのダイアログのまま |
...
| Model | Scene/Shot/Panel、整数フレーム、v3検証、v1→v2→v3 Migration、Asset参照、ストローク属性、Panel移動 | Project設定UI、音声素材、レイヤー |
| 編集 | Panel追加/複製/複数選択/Shift範囲選択/ドラッグ順序変更（Shot・Sceneをまたぐ移動を含む）/削除、Shot分割/統合、Scene・Shot名編集 | Panelのコピー&ペースト、テンプレート、検索 |
...
### P2 — Timeline EngineとCamera Track（次回の主対象）
...
2. `npm test`と`npm run build`で開始時点を確認。現在のUIを操作してTimelineの不便（目盛がない、再生ヘッドを追わない、Cameraキーが開始/終了しかない）を再現する。
3. P2はまず`src/app.js`のTimeline計算をTimeline Engineへ出す。範囲・スナップ・リップル規則を一か所に集めてから、目盛・追従スクロール・Cameraトラックを足す。
4. P2の完了条件を満たしたらP3へ進む。時間が残る場合もフェーズを飛び越えて機能を増やさない。
