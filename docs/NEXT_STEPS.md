...
更新日：2026-09-14。初期実装 `8b928b0` を基準に作成し、P0（保存・復旧とデータ基盤）、P1（描画とPanel操作）、P2（Timeline EngineとCamera Track）、P3（音配置・波形・同期再生）の実装後に状態を更新した。これは今後の実装計画であり、未実装機能の完成報告ではない。次の対象はP4（紙コンテを本番出力へ）。
...
| 音声波形とCameraキー | 同一時刻で再生・スクラブ・書き出し | Cameraキー・音声クリップとも編集可。波形はキャッシュして表示。動画書き出しはP5 |
...
| Model | Scene/Shot/Panel、整数フレーム、v4検証、v1→v4 Migration、Asset参照、ストローク属性、Panel移動、音声クリップ | Project設定UI、紙面設定、レイヤー |
...
| 出力 | PNG連番、印刷経由の画像ベースPDF | 用紙設定・長文継続・進捗/キャンセル（P4）、ネイティブPDF、Animatic動画 |

Inspectorの「SE / BGM（注記）」欄は文字注記で、音声クリップとは別のデータ（紙コンテでは両方を出す）。ブラウザ再生は動画ファイル書き出しではない。現在のUIを完成画像と同等と扱わない。自動保存はブラウザのIndexedDBであり、OS上のファイルへは保存ボタンで別途書き出す必要がある。
...
2. `npm test`と`npm run build`で開始時点を確認。紙コンテ出力を開いて、用紙がA4縦固定で、長文が「…」で切れ、500 Panelでは全ページを同期に走査することを確認する。
3. P4は紙面設定をプロジェクトへ持たせるところから始め、Paper RendererとExporterを分け、長文の継続とページ単位の進捗/キャンセルを足す。
4. P4の完了条件を満たしたらP5へ進む。時間が残る場合もフェーズを飛び越えて機能を増やさない。
