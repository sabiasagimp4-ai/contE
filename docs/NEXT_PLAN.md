...
## 2026-09-20 更新：UXレビューを先行実装

[UX_REVIEW_FIXES.md](UX_REVIEW_FIXES.md)に12項目の対応と検証を記録した。保存・入力保護、表示/編集対象の統一、狭い画面のInspector、Camera枠操作、紙面列順、Timelineの見渡し、通知を先行して修正した。N1は既存batchトランザクションとrevisionガードで実装し、下記のstaging案は採用しなかった。

直近の残件はN2（1000〜5000 Panel＋実素材の計測）。N5のCamera直接操作は先行対応済みで、コピー&ペースト・キー複製・イージング等は引き続き未完了。N3以降の採否は実測で決める。

以下は2026-09-19に作成した計画の基準状態と設計案を保持したもの。実装状態は上記更新とUX_REVIEW_FIXESを優先する。
...
2026-09-20時点の直近対象はN2。N1とUX修正の検証を維持して大規模実測へ進む。
