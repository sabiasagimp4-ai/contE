...
作成日: 2026-09-16
状態: A0（Timelineのドラッグ並べ替え）のみ実装済み。A1以降はすべて未着手。
A0の実績を各項目の見積りの基準にしている。
...
| A1 | コマの左端（境界）ドラッグ | 小 | 不要 | A0 |
| A2 | 数値ドラッグ中のライブ表示 | 中 | 不要 | なし |
| A3 | スナップ対象の追加とガイド線 | 中 | 不要 | なし |
| A4 | ドラッグ中の自動スクロール | 小 | 不要 | A0 |
