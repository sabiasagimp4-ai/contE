...
| A5 | 選択の拡張（Shift/Ctrl＋矢印） | 小 | 不要 | 実装済み |
| A6 | Sceneの開閉状態を覚える | 小 | 不要 | 実装済み |
| A7 | Undo/Redoの内容表示 | 小 | 不要 | 実装済み |
| A8 | 数値のホイールとTab移動 | 小 | 不要 | 実装済み |
| A9 | Timelineの行の高さ切り替え | 中 | 不要 | 実装済み |
| A10 | Cameraキーの複数選択 | 中 | 不要 | 実装済み |
| B1 | 尺の管理（総尺・目標・内訳） | 小 | 不要 | 実装済み |
| B2 | 台詞・注記の検索 | 中 | 不要 | 実装済み |
| B3 | 範囲再生・Shotループ | 中 | 不要 | 実装済み |
| B4 | Cameraキーのコピー／貼り付け | 小 | 不要 | 実装済み |
| B5 | 画像の差し替え | 小 | 不要 | 実装済み |
| B6 | 尺の一括設定・等分 | 小 | 不要 | 実装済み |
| B7 | 直線・矩形・矢印ツール | 中 | 不要 | 実装済み |
| B8 | プレゼンモード | 中 | 不要 | 実装済み |
| B9 | 保存履歴から選んで復元 | 中 | 不要 | 実装済み |
| B10 | 複製・貼り付けで音も持ち運ぶ | 中 | 不要 | 実装済み |
| C1 | マーカー | 中 | **v6** | 実装済み |
| C2 | コマのラベル色 | 小 | **v6** | 実装済み |
| C3 | Cameraのイージング | 中 | **v6** | 実装済み |
...
| E1 | 保存を止めない（分割シリアライズ） | 中 | 不要 | 実装済み |
| E2 | 保存・GCの直列化（R05） | 中 | 不要 | 実装済み |
| E3 | Animatic出力の内容固定 | 小 | 不要 | 実装済み |
| E4 | 複数タブの排他 | 中 | 不要 | 実装済み |
| U1 | 狭い画面の指定をflex前提に直す | 小 | 不要 | 実装済み |
| U2 | ファイル操作のメニュー化とメニューの部品化 | 小 | 不要 | 実装済み |
| U3 | プレゼンモードを最大化の上に載せる | 小 | 不要 | 実装済み |
| U4 | 作業レイアウトのプリセット | 小 | 不要 | 実装済み |
| U5 | 紙コンテをパネルにする | 大 | 不要 | 実装済み |
| U6 | Animaticをパネルにする | 中 | 不要 | 実装済み |
...
**Tab移動** 数値入力は素のDOM順（X→Y→Zoom→Rotationなど）に並んでおり、
`tabindex`もキー入力のフックも無い。ホイールの追加もkeydownには触れないので、
既存のTabでの移動は変わらず動く。追加の実装は不要（確認のみ）。
...
**実装メモ** `.rowName`/`#rowNames`が2か所で重複定義されており、
`top`だけ後勝ちで2〜4px分ずれていた（`.clip`は28pxなのに`.rowName.clips`は
実際には30pxで描画されていた）。カスタムプロパティへ移す際にこの重複を消し、
全ての`top`/`height`を`--off-clip`・`--off-camera`・`--off-audio`という
派生プロパティ経由にしたので、ずれの原因ごと無くなった。
...
browser smoke: Shiftクリック・矩形ドラッグで複数選び、選択している1本を
ドラッグすると選択全部が同じ量だけ動くこと。

**実装メモ** `moveCameraKeys`は複数キーをそれぞれ`moveCameraKey`（既存の
単体版、絶対t・衝突で上書き）へ振り分ける。同じ選択内の2本が重なるのを
避けるため、動く向きの先頭（進行方向で最もtが大きい/小さいキー）から順に
適用する（逆順だと自分の仲間の位置へ一時的に重なり、衝突処理に消される）。
...
**実装メモ** `playRange()`が範囲を決める：複数Panel選択中は
`tl.selectionRange`の範囲、それ以外はループのときだけ現在のShot全体、
どちらでもなければ現状通り（現在位置〜末尾、Undo等の既存挙動に影響しない）。
ループ用の`<input id="loop">`は`nav`に置き、Projectにもlayoutにも保存しない。
...
2. **実装済み**：C2 ラベル色（小）— Tree・Strip・Timelineの色分け。
   `setPanelLabel`Commandと、Inspectorの色スウォッチ（`#labelSwatches`）を足した。
3. **実装済み**：C1 マーカー（中）— Timelineの新しい行（`#markerTrack`）、
   Inspectorの新タブ、紙面への注記列（`marker`）。`src/markers.js`へ
   音声Clipと同じanchor+atの規則（長さは持たない一点）で実装した。
4. **実装済み**：C5 ワークエリア（中）— B3の範囲再生をここへ接続し、出力の既定範囲にした。
   `setWorkArea`Commandと、TimelineのIN/OUT/解除ボタン・帯表示（`#workAreaBand`）、
...
6. **実装済み**：C3 イージング（中）— Inspectorのカメラタブへ緩急セレクト
   （`#keyEase`）を追加し、`setCameraValues`Commandを再利用してeaseだけを
   差し替えられるようにした。`describeCamera`の表記は変えていない
   （動きの種類の判定には影響しないため）。
7. **実装済み**：C6 紙面の縦書き（中）— `paper.js`の`pageGeometry`が
   `paper.vertical`のとき列を右から左へ並べ替える（読む順＝columns配列の順は
   そのまま、向きだけ反転）。折返しは横書きの幅基準（`wrapLines`）に対して
   高さ基準の`wrapLinesVertical`を新設し、ページ内で収まる本数の基準も
   横書きの行高（`rowH`）から列幅へ入れ替えた（軸を入れ替えた対）。
   `renderPage`は1文字ずつ下へ送って1本の縦書きの列を描く。設定ダイアログに
   「縦書き」チェックボックスを追加し、`updatePaper`Commandをそのまま使う。
...
| **S11 画面の作り** | U0, U1〜U6（§11） | 要らないパネルを閉じられる。紙面とAnimaticを見ながら編集できる |
...
A〜E群で機能が揃った時点で、画面は「全部を常に出しておく」形のままだった。上の
バーに40近い操作が並び、左ツリー・右インスペクタ・下タイムラインは幅を変えられる
だけで一つも閉じられない。Stageの下にはショートカットの一覧が1行で貼り付いていて、
`white-space: nowrap`で途中から切れていた。読めない上に場所だけ取り、描く面が
そのぶん狭かった。

**ねらいは「必要なパネルを必要なときに出す」こと**で、多機能より使いやすさを取る。
AEのワークスペースに倣い、画面をドックとパネルに分ける。

### U0 パネル式UIの土台（実装済み）

- `src/ui/docks.js`：どのパネルを開き、どれを前面にするかだけを持つ。DOMとの約束は
  `[data-dock]` / `[data-tabs]` / `[data-body]` / `[data-splitter]`の4つ。`tabs`の
  ドックは1枚ずつ切り替え、`stack`のドックは開いている分を上から積む。中身が無く
  なったドックは仕切りごと消え、`fixed`のパネル（ビュー）は閉じられない。
- `src/ui/shortcuts.js`：効くキーと画面に出す説明を1つの表から配る。表と実装が
  別々にあると、片方だけ直したときに説明のほうが嘘になる。キーの要らない操作
  （ドラッグ・ホイール）は`GESTURES`に置く。
- 常時表示だったショートカットの行は「ショートカット」パネルへ移した。
- この作り替えで見つかった不具合も同時に直した：Ctrl+FがFの割り当て（Timelineの
  全体表示）まで一緒に起こしていたこと、`.panel`がドックの枠とツリーのコマの両方で
  使われ枠の指定が漏れていたこと、保存された配置を検算せず取り込んでいたこと、
  通知が36文字で切られて続きを読めなかったこと。

### U1 狭い画面の指定をflex前提に直す（実装済み）

`main`をflexにしたとき、`@media (max-width: 900px)`の`grid-template-columns`が
残っていた。こちらは効かないが、同じブロックの`#inspector { display: none }`は
効いており、幅900px以下ではウィンドウメニュー上「開いている」パネルが画面に
出なかった。パネルの開閉はDocks一本に決めたので、CSSが勝手に消す側を外す。
両脇のドックは`flex: 0 1`にして、入り切らないときは縮める（縮まない指定のままだと
横へはみ出す）。Stageは最後まで240pxを譲らない。

### U2 ファイル操作のメニュー化とメニューの部品化（実装済み）

ヘッダーに5つ並んでいた保存・開く・素材ごと書き出す・Bundleを開く・履歴は、名前
だけでは違いが分からなかった。「ファイル ▾」1つへ畳み、メニューの中で拡張子まで
書く。idも押したときの処理も動かしていないので、挙動は同じまま名前と置き場所だけ
が変わる。開閉・位置決め・外側クリックで閉じる処理は`src/ui/menu.js`へ出し、
ウィンドウメニューと共有する。項目を選んだときに閉じるのもメニュー側で受ける
（項目ごとに書かせると、1つ書き忘れたときにメニューが出たままになる）。

### U3 プレゼンモードを最大化の上に載せる（実装済み）

プレゼン（B8）は「nav・左右のパネル・仕切り・Timelineを消す」指定を自前で持って
いたが、それは`~`の最大化そのものだった。`docks.maximize("center")`を呼び、足すのは
ヘッダー・ツール列・コマを隠す分だけにする。Escは、プレゼン→最大化の順に解く
（先に最大化だけ解くと、ヘッダーを隠したままの中途半端な画面が残る）。

**最大化はlayoutへ保存しない。** 「今だけ広げて見る」操作なので、次に開いたときに
畳まれた画面で始まると何が起きたのか分からない。プレゼンが最大化の上に乗ったこと
で、保存すると「プレゼンのまま閉じたら次も畳まれている」状態も作れてしまう。

### U4 作業レイアウトのプリセット（実装済み）

やることが変わるたびに何枚も開き直すのは手間なので、ウィンドウメニューの先頭へ
3つ置く。**描く**（Timelineを畳んで面を広く取る）／**尺を決める**（Timelineを高くし、
音とマーカーを出す）／**仕上げ（既定）**（全部開いた初期状態。閉じすぎたときの
逃げ道も兼ねる）。

パネルの表・メニューの並び・作業レイアウトは`src/ui/panels.js`のデータにした。
ここで効く条件は**閉じられるパネルが必ずウィンドウメニューに載っていること**で、
載せ忘れたパネルは一度閉じると戻す手段が無くなる。DOMを持たないデータなら
ブラウザを起こさずに確かめられる（`tests/panels.test.js`）。

### U5 紙コンテをパネルにする（実装済み）

紙面を見ながら台詞を直せなかった。モーダルの間は編集どころかキー操作も止まり、
そもそもプレビューは用紙設定を変えたときしか作り直していなかったので、台詞を
直しても紙面は変わらなかった。

中央のドックをタブにして、コンポジション（ツール列・ビュー・コマ）と紙コンテを
同じ枠へ並べる。追従は**開いている間だけ**に限る：`layoutPages()`は全Panelを走る
ので、閉じたまま打つたびに走らせるとコマが増えるほど重くなる。出力中も作り直さ
ない（出力自体は開始時点のページを持っているので中身は混ざらないが、見ている
ページが勝手に入れ替わる）。用紙の設定欄はUndoで戻ったときに値がずれるので作り直す
が、その欄を触っている最中はそのままにする（打ち込みの途中でフォーカスが飛ぶ）。

印刷は`body`直下の`#printArea`へページを差し込んでから印刷する。以前の指定は
「`#paperDialog`の中の`#pages`だけを出す」という入れ子に依存していて、紙面の置き場所
が変わると成立しない。用紙の大きさは`PAPER_SIZES[].mm`と向きから`@page`として
書き出すので、印刷ダイアログで用紙を手で合わせる手順が要らなくなる。

ページ数の表示は`#status`から紙面のパネルへ移した。編集のたびに`#status`を上書き
すると、保存の失敗のような他の知らせが消えてしまう。

### U6 Animaticをパネルにする（実装済み）

同じ枠の3枚目のタブ。開いているあいだは見積り（尺・フレーム数・解像度）が今の
Projectに追従する。形式やfpsの選択肢は作り直さない（選んだものが既定へ戻る）。

録画中の扱いだけ紙コンテと変える。紙面の出力は開始時点のページを持っているだけ
なので編集を止める理由が無いが、録画は実時間で進むので、途中でProjectや再生ヘッドが
動くと映像も音も作り直しになる。`body[data-recording]`を立ててAnimaticのパネル以外
を触れなくし、キー操作も止める。中止だけはいつでも押せる。入力欄に残っていた
フォーカスは開始時に外す（触れない面の上でも打てるため）。

**止めるのは人の操作だけにした。** 開始前に走り出した取り込みを途中で拒否すると、
E3（出力の内容は開始時点で固定する）が守っている「出力は固定するが編集自体は通る」
という筋が崩れる。出力へ写り込まないことは固定のほうで保証されている。

### U群でやらないと決めたこと

- **コマの一覧を3か所（ツリー／サムネイル／Timelineのコマ行）から減らすこと。**
  役割は既に分かれており（ツリー＝構造、コマ＝絵、Timelineのコマ行＝尺）、要らない
  ものは閉じられるようになった時点で解決とみなす。既定は変えない。
- **復旧ダイアログのモーダル解除。** 起動時に「復旧するか破棄するか」を決めて
  もらう場面なので、ほかの操作を止めるのが正しい。
- **`buildBundle`の全か無か。** 素材が1つ欠けたときに書き出し全体を止めるのは、
  壊れたBundleを作らないための正しい判断。変えるならエラーメッセージだけ。
