...
- **パネル式のUI**。左（プロジェクト）・中央（コンポジション / 紙コンテ / Animatic）・右（内容 / Camera / 音 / マーカー / 構成 / ショートカット）・下（タイムライン）に分かれ、使わないパネルはタブの×か「ウィンドウ」メニューで閉じられます。閉じると画面から消え、同じメニューから戻せます。境界のドラッグで幅/高さを変えられ、開閉と大きさは次回の起動でも保たれます。`~`（またはタブのバーをダブルクリック）で、いま触っているパネルだけを画面いっぱいに広げられます（Escで戻る）。最大化は覚えません（次に開くと元の配置に戻ります）。「プレゼン」はこの最大化の上に、ヘッダー・ツール列・コマも隠してフルスクリーンにしたものです。ウィンドウメニューの先頭には作業レイアウト（描く / 尺を決める / 仕上げ）があり、やることに合わせてパネルの開閉と幅・高さをまとめて切り替えられます。閉じすぎたときは「仕上げ（既定）」で戻せます。
...
- **Animatic出力**：音つきWebM（実時間で録画）と、フレーム厳密なPNG連番ZIP（音なし・外部エンコーダ用）。出力fpsと解像度（480p/720p/1080p）を選べ、進捗表示と中止に対応します。中央の「Animatic」タブに出ます。録画中は他の操作を止めます（録画は実時間で進むため）が、中止はいつでも押せます。映像は再生とまったく同じフレーム評価（Panel境界・Camera補間）で描き、音は再生と同じ予約を録音経路へ流します。
- 紙コンテは中央の「紙コンテ」タブに出ます。**開いたまま台詞や尺を直せて、紙面はそれに追従します**（閉じている間は作り直しません）。出力は**PNG連番（1つのZIPにまとめてダウンロード）**とブラウザ印刷/PDF保存。1ページずつ生成して進捗を表示し、途中で中止できます。用紙サイズと向きは印刷にもそのまま渡すので、印刷ダイアログでは余白なし・ヘッダー/フッターなしだけ選んでください。PDFは画像ベースで文字検索できません。
